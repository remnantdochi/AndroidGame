...
public class Shoot extends JFrame implements Runnable, KeyListener {
 ...
 public void plCreate() {
	  for(int i = 0; i < 9; i++) {
	   double rx = Math.random() * (w - xw);
	   double ry = Math.random() * 50;
	   Plus pl = new Plus((int)rx, (int)ry);
	   plList.add(pl);
	  }
 }
 public void crashChk() {
  ...
  for(int i = 0; i < plList.size(); i++) {
	   Plus pl = (Plus)plList.get(i);
	   int[] xpoints = {x, (x + xw), (x + xw), x};
	   int[] ypoints = {y, y, (y + xh), (y + xh)};
	   p = new Polygon(xpoints, ypoints, 4);
	   if(p.intersects((double)pl.x, (double)pl.y, (double)pl.w, (double)pl.h)) {
	    plList.remove(i);
	    point += 1;
	   }
}
 ...
 public void draw() {
  ...
  for(int i = 0; i < plList.size(); i++) {
   Plus pl = (Plus)plList.get(i);
   gs.fillRect(pl.x, pl.y, pl.w, pl.h);
   if(pl.y > h) plList.remove(i);
   pl.moveEn();
  }
